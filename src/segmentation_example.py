...
class CameraMouse(object):
    ...
    # create trackbars for color change
        tunersAreCreated = True
        cv2.namedWindow(hsv_tuning)
        cv2.createTrackbar('H_MIN',hsv_tuning,self.hmin,255,self.nothing)
        cv2.createTrackbar('S_MIN',hsv_tuning,self.smin,255,self.nothing)
        cv2.createTrackbar('V_MIN',hsv_tuning,self.vmin,255,self.nothing)
        cv2.createTrackbar('H_MAX',hsv_tuning,self.hmax,255,self.nothing)
        cv2.createTrackbar('S_MAX',hsv_tuning,self.smax,255,self.nothing)
        cv2.createTrackbar('V_MAX',hsv_tuning,self.vmax,255,self.nothing)

        cv2.namedWindow(th_window_name)
        cv2.createTrackbar('Th_min', th_window_name, self.th_min, 255, self.nothing)
        cv2.createTrackbar('Th_max', th_window_name, self.th_max, 255, self.nothing)
    ...
    def color_treshold(self,img):
        ...
        if self.tunersAreCreated:
            self.hmin = cv2.getTrackbarPos('H_MIN',hsv_tuning)
            self.smin = cv2.getTrackbarPos('S_MIN',hsv_tuning)
            self.vmin = cv2.getTrackbarPos('V_MIN',hsv_tuning)
            self.hmax = cv2.getTrackbarPos('H_MAX',hsv_tuning)
            self.smax = cv2.getTrackbarPos('S_MAX',hsv_tuning)
            self.vmax = cv2.getTrackbarPos('V_MAX',hsv_tuning)
    ...
    def gray_treshold(self, img):
        if self.tunersAreCreated:
            self.th_min = cv2.getTrackbarPos('Th_min', th_window_name)
            self.th_max = cv2.getTrackbarPos('Th_max', th_window_name)
    ...
    def runProgram(self):
        while(True):
            ...
            ret, frame = self.cap.read()
            frame_no_bg = self.removeBackground(frame, showIO = False)
        #    edges = cv2.Canny(frame_no_bg,100,100)
        #    cv2.imshow('edges', edges)

            # Converting to HUE-SATURATION image
            segmentedImg = self.applySegmentationBasedonHSV(frame_no_bg, showIO = False)
            ...
            try:
                cx, cy, points = self.applyConvexHull(closing, originalImg = frame ,showIO = False)
